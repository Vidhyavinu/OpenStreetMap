...
float RoutePlanner::CalculateHValue(RouteModel::Node const *node) {
 ...
 // std::cout<<"Node: "<<node->x<<"\n";
 // if(end_node == NULL)
   //  std::cout<<"end_node is NULL.\n";
  float fval = node->distance(*end_node);
  return fval;
}
...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
  ...
  for(RouteModel::Node *neigh: current_node->neighbors)
  {
    ...
    neigh->g_value = current_node->g_value+
      				neigh->distance(*start_node);
...
// - Return the pointer.
bool compare(RouteModel::Node* p, RouteModel::Node* q)
{
  float g = p->g_value + p->h_value;
  float h = q->g_value + q->h_value;
